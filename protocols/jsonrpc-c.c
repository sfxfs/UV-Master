...
static int send_http_response(struct jrpc_connection * conn, char *response) {
	int fd = conn->fd;
	int header_len = strlen(jrpc_http_header);
	int reponse_len = strlen(response);
	char* tosend = (char*)malloc(header_len + 25 + reponse_len);	//25 relative to "Content-Length: %d\r\n\r\n"
	...
	if (write(fd, tosend, strlen(tosend) + 1))
		return 0;
	else
		return -1;
}
...
static int send_error(struct jrpc_connection * conn, int code, char* message,
	...
	cJSON_AddStringToObject(result_root, "jsonrpc", JRPC_VERSION);		//added by wangzugang for jsonrpc-c-2.0
	cJSON_AddItemToObject(result_root, "error", error_root);
	cJSON_AddItemToObject(result_root, "id", id);

	char * str_result = cJSON_PrintUnformatted(result_root);
	return_value = send_http_response(conn, str_result);
	free(str_result);
...
static int send_batch(struct jrpc_connection *conn, int count, jrpc_context *ctx, cJSON **returned, int *procedure_found,
		cJSON **id) {
	...
	for(i = 0; i < count; i++)
	{
		...
		{
			result_root = cJSON_CreateObject();
			cJSON_AddStringToObject(result_root, "jsonrpc", JRPC_VERSION);		//added by wangzugang
			if (returned[i])
			{
	...
	char * str_result = cJSON_PrintUnformatted(array);
	return_value = send_http_response(conn, str_result);
	free(str_result);
	cJSON_Delete(array);	
	free(ctx);
...
//added by wangzugang, this is added for jsonrpc-c-2.0
static int invoke_procedure_batch(struct jrpc_server *server, struct jrpc_connection * conn, int count,
		cJSON ** method, cJSON **params, cJSON **id) {
	cJSON **returned = NULL;
	...
	free(method);
	free(params);
	return send_batch(conn, count, ctx, returned, procedure_found, id);
}

static int eval_request(struct jrpc_server *server, struct jrpc_connection * conn,
		cJSON *root) {
...
static int eval_request_batch(struct jrpc_server *server, struct jrpc_connection * conn, cJSON *root) 
{
	...
	return invoke_procedure_batch(server, conn, count, method, params, idcopy);
}
...
static void connection_cb(struct ev_loop *loop, ev_io *w, int revents) {
	...
	while (1)
	{
		if (conn->pos == (conn->buffer_size - 1)) // buffer is full, realloc buffer for more space
		{
		...
		{
			// parse http request (add by sfxfs)
			const char *method, *path;
			int minor_version;
			struct phr_header headers[20];
			size_t prev_pos = 0, method_len, path_len, num_headers;

			prev_pos = conn->pos;
			conn->pos += bytes_read;
			num_headers = sizeof(headers) / sizeof(headers[0]);
			header_len = phr_parse_request(conn->buffer, conn->pos, &method, &method_len, &path, &path_len, &minor_version, headers, &num_headers, prev_pos);
			if (header_len > 0)
				break; // successfully parsed the request
			else if (header_len == -1)
				return close_connection(loop, w); // parse failed
		}
	}
	//parse json data in the body of http request
	cJSON *root;
	char *end_ptr = NULL;

	if ((root = cJSON_Parse_Stream(conn->buffer + header_len, &end_ptr)) != NULL)
	{
...
static void accept_cb(struct ev_loop *loop, ev_io *w, int revents) {
	...
	if (connection_watcher->fd == -1) {
		...
		connection_watcher->buffer_size = JRPC_INITIAL_BUFFER_SIZE;
		connection_watcher->buffer = malloc(JRPC_INITIAL_BUFFER_SIZE);
		memset(connection_watcher->buffer, 0, JRPC_INITIAL_BUFFER_SIZE);
