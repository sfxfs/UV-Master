...
int rpc_add_method (rpc_handle_t *handle, jrpc_function function_pointer, char *name, void * data)
{
	...
	if (!handle->procedures)
		...
		if (!ptr)
        {
            if (handle->debug_level > 0) printf("Cannot alloc mem for func %s\n", name);
			return -1;
        }
...
int rpc_del_method (rpc_handle_t *handle, char *name)
{
	...
	if (handle->procedures){
		...
		if (handle->debug_level > 0) printf("Method '%s' not found\n", name);
		return -1;
	}
	return 0;
}
...
static cJSON *rpc_ok (cJSON * result, cJSON * id)
{
	...
	if (result) cJSON_AddItemToObject(result_root, "result", result);
...
static cJSON *rpc_err (int code, char* message, cJSON * id)
{
    ...
    if (result_root == NULL || error_root == NULL) return NULL;
...
static cJSON *invoke_procedure (rpc_handle_t *handle, char *name, cJSON *params, cJSON *id)
{
	...
	if (!procedure_found) {
        if (handle->debug_level > 0) printf("Method not found: %s\n", name);
	...
	if (method != NULL && method->type == cJSON_String) {
		...
		|| params->type == cJSON_Object) {
			...
			//We have to copy ID because using it on the reply and deleting the response Object will also delete ID
				...
				if (handle->debug_level > 0) printf("Method Invoked: %s\n", method->valuestring);
...
{
	if (handle->debug_level > 1) printf("recv json str:\n%.*s\n", req_len, json_req);
    cJSON *request = cJSON_ParseWithLength(json_req, req_len);
    if (request == NULL)
    {
        if (handle->debug_level > 0) printf("Valid JSON Received\n");
